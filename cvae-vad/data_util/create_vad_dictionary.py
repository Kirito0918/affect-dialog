...
if __name__ == '__main__':
    ...
    print(f'初始化vad字典大小: {len(vocab_vad_dict)}')
    print(f'在nrc-vad-lexicon中的单词个数: {len(vocab_vad_dict) - num_not_in_vad}')
    print(f'不在nrc-vad-lexicon中的单词个数: {num_not_in_vad}')
    print(f'通过lemma词根初始化的个数: {num_lemma}')
    print(f'通过portor词根初始化的个数: {num_porter}')
    print(f'通过同义词初始化的个数: {num_syn}')
    print(f'通过lemma词根的同义词初始化的个数: {num_lemma_syn}')
    print(f'通过portor词根的同义词初始化的个数: {num_porter_syn}')
    print(f'通过标点初始化的个数: {num_symbols}')
    print(f'通过数字初始化的个数: {num_numbers}')
    print(f'匹配不到任何近义词而直接初始化的单词个数: {len(init_words)}')
    print(f'匹配不到任何近义词而直接初始化的单词: {init_words}')
    print('匹配不到任何近义词而直接初始化的单词占词汇表的{:.2f}%'.format(100.0*len(init_words)/len(vocab)))
    print(f'匹配不到任何近义词而直接初始化的单词包含停止词的个数: {num_in_stopwords}')
    print('匹配不到任何近义词而直接初始化的单词包含{:.2f}%的停止词'.format(100.0*num_in_stopwords/len(init_words)))
