...
parser.add_argument('--model_path', dest='model_path', default='log/run1577783126/013000001615000.model', type=str, help='载入模型位置')  #
    ...
    if not args.inference:
        ...
        while epoch < args.max_epoch:  # 最大训练轮数
            ...
            for data in dp_train.get_batch_data():
                ...
                loss, nll_loss, kld_loss, kld_weight, ppl = train(model, feed_data, global_step)
                ...
                if global_step % args.log_per_step == 0 or \
                    ...
                    model.train()
                    print('在验证集上的NLL损失为: {:g}, KL损失为: {:g}, PPL为: {:g}'.format(nll_loss, kld_loss, np.exp(ppl)))
...
def compute_loss(outputs, labels, masks, global_step):
    ...
    output_vocab, _mu, _logvar, mu, logvar = outputs
    ...
    # 1.nll损失
    _nll_loss = F.nll_loss(output_vocab.clamp_min(1e-12).log(), labels, reduction='none')  # [batch*len_decoder]
    ...
    return loss, nll_loss, kld_loss, kld_weight, ppl
...
def train(model, feed_data, global_step):
    ...
    labels = feed_data['responses'][:, 1:]
    masks = feed_data['masks']
    loss, nll_loss, kld_loss, kld_weight, ppl = compute_loss(outputs, labels, masks, global_step)  # 计算损失
    return loss, nll_loss, kld_loss, kld_weight, ppl
...
def valid(model, data_processor, global_step):
    ...
    for data in data_processor.get_batch_data():
        ...
        labels = feed_data['responses'][:, 1:]
        masks = feed_data['masks']
        _, nll_loss, kld_loss, kld_weight, ppl = compute_loss(outputs, labels, masks, global_step)  # 计算损失
