...
parser.add_argument('--model_path', dest='model_path', default='log/run1586267552/040000000190160.model', type=str, help='载入模型位置')  #
...
def main():
    ...
    if not args.inference:
        ...
        while epoch < args.max_epoch:  # 最大训练轮数
            ...
            for data in dp_train.get_batch_data():
                ...
                if global_step % args.print_per_step == 0:
                    print('epoch: {:d}, global_step: {:d}, lr: {:g}, rl_loss: {:.2f}, reward: {:.2f}, nll_loss: {:.2f},'
                          ...
                          .format(epoch, global_step, optim.lr, rl_loss.mean().item(), reward.mean().item(),
                                  nll_loss.mean().item(), kld_loss.mean().item(), kld_weight,
                ...
                if global_step % args.log_per_step == 0 \
                    ...
                    model.eval()
                    reward, nll_loss, kld_loss, ppl = valid(model, dp_valid, global_step-1)
                    model.train()
        ...
        reward, nll_loss, kld_loss, ppl = valid(model, dp_test, global_step-1)
...
def compute_loss(outputs, labels, masks, global_step):
    ...
    rl_loss = nll_loss + kld_weight * kld_loss + 1.0 * nll_loss * reward

    return rl_loss, _reward, loss, nll_loss, kld_loss, kld_weight, ppl


def train(model, feed_data, global_step):
    output_vocab, _mu, _logvar, mu, logvar = model(feed_data, use_true=args.use_true, gpu=args.gpu)
    with torch.no_grad():
    ...
    rl_loss, _reward, loss, nll_loss, kld_loss, kld_weight, ppl = \
        compute_loss(outputs, labels, masks, global_step)
    return rl_loss, _reward, loss, nll_loss, kld_loss, kld_weight, ppl


def valid(model, data_processor, global_step):
    rewards, nll_losses, kld_losses, ppls = [], [], [], []
    for data in data_processor.get_batch_data():
        ...
        _, reward, _, nll_loss, kld_loss, kld_weight, ppl = compute_loss(outputs, labels, masks, global_step)
    ...
    return rewards.mean(), nll_losses.mean(), kld_losses.mean(), ppls.mean()
