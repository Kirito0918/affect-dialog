...
parser.add_argument('--trainset_path', dest='trainset_path', default='data/raw/dialogues_train_manyturns.txt', type=str, help='训练集位置')
parser.add_argument('--validset_path', dest='validset_path', default='data/raw/dialogues_validation_manyturns.txt', type=str, help='验证集位置')
parser.add_argument('--testset_path', dest='testset_path', default='data/raw/dialogues_test_manyturns.txt', type=str, help='测试集位置')
...
def main():
    ...
    if not args.inference:
        ...
        while epoch < args.max_epoch:  # 最大训练轮数
            ...
            for data in dp_train.get_batch_data():
                ...
                if global_step % args.print_per_step == 0:
                    print('epoch: {:d}, global_step: {:d}, lr: {:g}, nll_loss: {:.2f}, ppl: {:.2f}, time: {:.2f}s/step'
                          .format(epoch, global_step, optim.lr, nll_loss.mean().item(), ppl.mean().exp().item(),
                                  use_time))
                ...
                if global_step % args.log_per_step == 0:  # 保存模型
                    ...
                    model.train()
                    print('在验证集上的NLL损失为: {:g}, PPL为: {:g}'.format(nll_loss, np.exp(ppl)))
        ...
        print('在测试集上的NLL损失为: {:g}, PPL为: {:g}'.format(nll_loss, np.exp(ppl)))
...
def prepare_feed_data(data, inference=False):
    ...
    if not inference:  # 训练时的输入
        ...
        feed_data = {'posts': torch.tensor(data['posts']).long(),
                     ...
                     'turn_posts': torch.tensor(data['turn_posts']).long()}
...
def train(model, feed_data):
    output_vocab = model(feed_data, gpu=args.gpu)  # 前向传播
    labels = feed_data['responses'][:, 1:]  # 去掉start_id
    masks = feed_data['masks']
    nll_loss, ppl = compute_loss(output_vocab, labels, masks)  # 计算损失
    return nll_loss, ppl
...
def valid(model, data_processor):
    ...
    for data in data_processor.get_batch_data():
        feed_data = prepare_feed_data(data)
        output_vocab = model(feed_data, gpu=args.gpu)
        labels = feed_data['responses'][:, 1:]  # 去掉start_id
        masks = feed_data['masks']

        nll_loss, ppl = compute_loss(output_vocab, labels, masks)
...
def test(model, feed_data):
    output_vocab = model(feed_data, inference=True, max_len=args.max_len, gpu=args.gpu)
