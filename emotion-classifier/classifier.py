...
parser.add_argument('--trainset_path', dest='trainset_path', default='data/sc_train.txt', type=str, help='训练集位置')
parser.add_argument('--validset_path', dest='validset_path', default='data/sc_valid.txt', type=str, help='验证集位置')
parser.add_argument('--testset_path', dest='testset_path', default='data/sc_test.txt', type=str, help='测试集位置')
...
def prepare_feed_data(data):
    feed_data = {'x': torch.tensor(data['texts']).long(),  # [batch, len_encoder]
                 'len_x': torch.tensor(data['len_texts']).long(),  # [batch]
                 'emotions': torch.tensor(data['emotions']).long()}  # [batch, len_decoder]
...
def test(model, feed_data):
    ...
    return output_vocab.argmax(1).detach().tolist()
