...
def main():
    ...
    if not args.inference:
        ...
        while epoch < args.max_epoch:  # 最大训练轮数
            ...
            for data in dp_train.get_batch_data():
                ...
                if global_step % args.print_per_step == 0:
                    print('epoch: {:d}, global_step: {:d}, lr: {:g},  reward: {:.2f}, nll_loss: {:.2f}, ppl: {:.2f},'
                          ' time: {:.2f}s/step'
                          .format(epoch, global_step, optim.lr, reward.mean().item(), nll_loss.mean().item(),
                ...
                if global_step % args.log_per_step == 0:  # 保存模型
                    ...
                    model.train()
                    print('在验证集上的REWARD为: {:g}, NLL损失为: {:g}, PPL为: {:g}'.format(reward, nll_loss, np.exp(ppl)))
        ...
        print('在测试集上的REWARD为: {:g}, NLL损失为: {:g}, PPL为: {:g}'.format(reward, nll_loss, np.exp(ppl)))
...
def compute_loss(outputs, labels, masks):
    ...
    rl_loss = nll_loss + 1.0 * nll_loss * reward

    return rl_loss, _reward, nll_loss, ppl
...
def train(model, feed_data):
    ...
    rl_loss, reward, nll_loss, ppl = compute_loss(outputs, labels, masks)  # 计算损失
    return rl_loss, reward, nll_loss, ppl


def valid(model, data_processor):
    rewards, nll_losses, ppls = [], [], []
    for data in data_processor.get_batch_data():
        ...
        labels = (labels_word, labels_affect)
        masks = feed_data['masks']
        _, reward, nll_loss, ppl = compute_loss(outputs, labels, masks)
    ...
    ppls = np.array(ppls)
    return rewards.mean(), nll_losses.mean(), ppls.mean()
